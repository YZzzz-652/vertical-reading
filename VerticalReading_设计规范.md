...
在三主色基础上，可根据 UI 需要派生中间色，用于副标题、meta 文字、分隔线、不同透明度的叠加面板等场景。派生色不作主色使用，命名建议体现"从哪一主色派生 + 用途"。当前已纳入 token 体系的派生色：

| 变量名 | 色值 | 派生自 | 用途 |
|------|------|------|------|
| `--fg-muted` | `#6c532e` | ink | 副标题、legend |
| `--fg-soft` | `#71572e` | ink | meta、标签文字 |

新增派生色时遵循同样命名规则，并同步更新本文档。
