...
**文件位置**：`AGENTS.md` 在仓库根目录；其余 7 份文档全部在 `working-docs/` 目录下。
...
### 启动流程
每次新对话开启时（两段式：先看待办、再挑任务、后补文档）：
1. 用户上传**脊柱两份**——根目录 `AGENTS.md` + `working-docs/VerticalReading_项目启动文档.md`，开场只需这两份
2. 读完先输出 /status 快照（当前阶段、数据进度、当前待办、下一步优先级），把待办列出来供用户挑选
3. 用户从待办挑定任务后，按 §5 路由表判断该任务还需哪些文档，提示用户补传（缺则提示，不凭记忆硬答）
4. 文档补齐后开展工作，等待指令
（回顾 / 收尾类任务，用户同时粘贴 Codex 端 `git log --oneline -10` 与 `git status`）
...
2. 只读**脊柱两份**：根目录 `AGENTS.md` + `working-docs/VerticalReading_项目启动文档.md`，不预先读其余 MD
3. 输出 /status（当前阶段、数据进度、当前待办、下一步优先级），把待办列出来供用户挑选
4. 用户挑定任务后，按 §5 路由表读取 `working-docs/` 下该任务对应的文档
5. 等待指令；未挑定任务前不改任何文件
